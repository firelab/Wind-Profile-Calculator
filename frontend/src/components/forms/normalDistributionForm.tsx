...
export default function NormalDistributionForm({ onSubmit }: { onSubmit: (data: any) => void }) {
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post('http://localhost:5000/normal_distribution', formData);
    ...
    return (
        <Box display="flex" flexDirection="column" gap={2}>
            ...
            <TextField required id="standardDevFoliageDist" label="Std Dev of Foliage" type="number" onChange={handleChange} />
